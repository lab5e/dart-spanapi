...
class ClearFirmwareErrorResponse {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "ClearFirmwareErrorResponse[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "ClearFirmwareErrorResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ClearFirmwareErrorResponse>? listFromJson(
    ...
    final result = <ClearFirmwareErrorResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ClearFirmwareErrorResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ClearFirmwareErrorResponse> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ClearFirmwareErrorResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ClearFirmwareErrorResponse>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ClearFirmwareErrorResponse.listFromJson(
          entry.value,
          growable: growable,
        );
        if (value != null) {
