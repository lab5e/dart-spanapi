...
class ListFirmwareResponse {
  ...
  static ListFirmwareResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "ListFirmwareResponse[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "ListFirmwareResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ListFirmwareResponse(
        images: Firmware.listFromJson(json[r'images']) ?? const [],
      );
    }
    return null;
  }
  ...
  static List<ListFirmwareResponse>? listFromJson(
    ...
    final result = <ListFirmwareResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ListFirmwareResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ListFirmwareResponse> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ListFirmwareResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ListFirmwareResponse>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ListFirmwareResponse.listFromJson(
          entry.value,
          growable: growable,
        );
        if (value != null) {
