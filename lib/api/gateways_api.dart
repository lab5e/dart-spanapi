...
class GatewaysApi {
  ...
  Future<Response> listGatewaysWithHttpInfo(String networkId,) async {
    // Verify required params are set.
    if (networkId == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: networkId');
    }
  ...
  Future<ListGatewayResponse> listGateways(String networkId,) async {
    final response = await listGatewaysWithHttpInfo(networkId,);
  ...
  Future<ListNetworkResponse> listNetworks() async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body != null && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ListNetworkResponse',) as ListNetworkResponse;
  ...
  Future<Response> retrieveGatewayWithHttpInfo(String networkId, String gatewayId,) async {
    // Verify required params are set.
    if (networkId == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: networkId');
    }
  ...
  Future<Gateway> retrieveGateway(String networkId, String gatewayId,) async {
    final response = await retrieveGatewayWithHttpInfo(networkId, gatewayId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
