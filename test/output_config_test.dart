//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:spanapi/api.dart';
import 'package:test/test.dart';

// tests for OutputConfig
void main() {
  // final instance = OutputConfig();

  group('test OutputConfig', () {
    // URL for the webhook.
    // String url
    test('to test the property `url`', () async {
      // TODO
    });

    // String basicAuthUser
    test('to test the property `basicAuthUser`', () async {
      // TODO
    });

    // String basicAuthPass
    test('to test the property `basicAuthPass`', () async {
      // TODO
    });

    // String customHeaderName
    test('to test the property `customHeaderName`', () async {
      // TODO
    });

    // String customHeaderValue
    test('to test the property `customHeaderValue`', () async {
      // TODO
    });

    // String host
    test('to test the property `host`', () async {
      // TODO
    });

    // int port
    test('to test the property `port`', () async {
      // TODO
    });

    // String key
    test('to test the property `key`', () async {
      // TODO
    });

    // String eventName
    test('to test the property `eventName`', () async {
      // TODO
    });

    // bool asIsPayload
    test('to test the property `asIsPayload`', () async {
      // TODO
    });

    // String endpoint
    test('to test the property `endpoint`', () async {
      // TODO
    });

    // MQTT configuration: Disable certificate checks. Default is off.
    // bool disableCertCheck
    test('to test the property `disableCertCheck`', () async {
      // TODO
    });

    // String username
    test('to test the property `username`', () async {
      // TODO
    });

    // String password
    test('to test the property `password`', () async {
      // TODO
    });

    // String clientId
    test('to test the property `clientId`', () async {
      // TODO
    });

    // String topicName
    test('to test the property `topicName`', () async {
      // TODO
    });

    // String topicTemplate
    test('to test the property `topicTemplate`', () async {
      // TODO
    });

    // String payloadFormat
    test('to test the property `payloadFormat`', () async {
      // TODO
    });

    // String payloadTemplate
    test('to test the property `payloadTemplate`', () async {
      // TODO
    });
  });
}
